...
import { listProfiles, upsertProfile, deleteProfile, formatProfileList, } from "./profiles.js";
import { MODES } from "./modes.js";
const MODE_VALUES = Object.keys(MODES);
const server = new Server({ name: "presend", version: "0.1.0" }, { capabilities: { tools: {} } });
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        {
            name: "check_message",
            description: "Analyze a message before sending. Returns a score, red/yellow/green flags, and a one-line summary. Use this when the user says they're about to send an email, post, proposal, or feedback and wants a sanity check.",
            inputSchema: {
                type: "object",
                properties: {
                    ...
                    context: {
                        type: "string",
                        description: "Optional free-text context: who the recipient is, the situation, the goal.",
                    },
                    profile_id: {
                        type: "string",
        ...
        {
            name: "fix_message",
            description: "Apply check_message flags and return a corrected version of the message.",
            inputSchema: {
                type: "object",
                properties: {
        ...
        {
            name: "save_profile",
            description: "Create or update a communication profile describing a recipient (boss, client, etc.) and how they prefer to be communicated with.",
            inputSchema: {
                type: "object",
                properties: {
                    id: {
                    ...
                    description: {
                        type: "string",
                        description: "Free text: who they are, relationship, communication style, sensitivities.",
                    },
                    language: {
                        type: "string",
        ...
        switch (name) {
            case "check_message": {
                ...
                if (!MODE_VALUES.includes(mode))
                    return errorResult(`mode must be one of: ${MODE_VALUES.join(", ")}`);
            ...
            case "fix_message": {
                ...
                const mode = String(args.mode ?? "");
                if (!text || !flags)
                    return errorResult("text and flags are required");
                if (!MODE_VALUES.includes(mode))
                    return errorResult(`mode must be one of: ${MODE_VALUES.join(", ")}`);
                const ctx = args.context ? String(args.context) : undefined;
                const pid = args.profile_id ? String(args.profile_id) : undefined;
